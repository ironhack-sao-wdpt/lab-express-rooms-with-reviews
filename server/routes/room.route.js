...
router.post("/room", async (req, res) => {
  try {
    ...
    console.error(err);
    return res.status(500).json({ msg: "Falha ao cadastrar imóvel" });
  }
});
...
router.get("/room", async (req, res) => {
  try {
    ...
    console.error(err);
    return res.status(500).json({ msg: "Falha ao buscar imóveis" });
  }
});
...
router.patch("/room/:_id", async (req, res) => {
  try {
    ...
    console.error(err);
    return res.status(500).json({ msg: "Falha ao editar imóvel" });
  }
});
